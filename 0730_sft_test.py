    ...
    if training_args.use_peft:
        ...
        if training_args.peft_path is not None:
            ...
            if target_modules and 'all' in target_modules:
                target_modules = find_all_linear_names(model, int4=True if training_args.qlora else False, int8=model_args.load_in_8bit)
            modules_to_save = training_args.modules_to_save
            if modules_to_save is not None:
                modules_to_save = modules_to_save.split(',')

            model = prepare_model_for_kbit_training(model, use_gradient_checkpointing=True) ## add   要写在加载loraConfig之前
