...
def main():
    ...
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()  ##################20230725  model_args, data_args, training_args 三个参数集合全场都可以用！所以后面main函数里面随处可见
    ...
    if model_args.model_name_or_path:
        ...
        if training_args.qlora: # 启用qlora
    ...
    if model_args.model_type !="chatglm" : #####20230725  chatglm好像不能显式设置
        tokenizer.pad_token = tokenizer.unk_token if model_args.model_type !="chatglm" else "<unk>"   ####20230725#### https://huggingface.co/THUDM/chatglm2-6b/blob/main/tokenization_chatglm.py#L91
    ...
    if training_args.use_peft:
        ...
        #if training_args.qlora :   ########## 这个prepare_model_for_kbit_training(model) 之后可训练参数成了0 有bug
        #    logger.info("模型使用qlora 开始prepare_model_for_kbit_training，从而在训练时减少显存消耗。")
    ...
    def preprocess_function(examples):
        ...
        for conversation, target in zip(conversations, targets):
            ...
            for i, turn in enumerate(turns):
                ...
                logger.warning(f"turn_len: {turn_len}")
                parts = turn.split(sep)
                logger.info(f"len(parts)={len(parts)} , parts={parts}")
                if len(parts) != 2:
                    break
                logger.info(f"after break I am here!!!!")
                parts[0] += sep
                ...
                cur_len += turn_len
                logger.warning(f"in loop cur_len: {cur_len}")
            target[cur_len:] = IGNORE_INDEX
    ...
    if not ddp and torch.cuda.device_count() > 1  and model_args.model_type !="chatglm" :  ####20230725  也就是说ddp表示worldsize>1时候不用trainer自己的分布式训练 而是用deepspeed torchrun的
        # Keeps Trainer from trying its own DataParallelism when more than 1 gpu is available
        model.is_parallelizable = True
        model.model_parallel = True 
        logger.warning(f"model.is_parallelizable={model.is_parallelizable},model.model_parallel={model.model_parallel}")
